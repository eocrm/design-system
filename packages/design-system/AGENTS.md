...
- **`unavailable` is announced, not just muted.** A localized word is rendered visually hidden inside the chip, so the accessible name reads `"Appointment (unavailable)"`. This matters because the canonical use is to withhold the entity's name and show a TYPE word instead — without it, a masked reference is indistinguishable from a real entity that happens to be called "Appointment". `aria-disabled` alone does not do this: on a role-less `<span>` it is not exposed by screen readers. Override the word via `<I18nProvider overrides={{ entityChip: { unavailable: '…' } }}>`; it carries its own punctuation so a locale can pick different marks.
