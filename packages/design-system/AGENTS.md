...
- **Column header names.** The `<th>` is named by the label span's content — the header text — so the resize handle is never folded into it (#500). The one override: a **ReactNode** header WITH a `visibilityLabel` is named by that label instead. So `header: <strong>Revenue</strong>` announces "Revenue", and adding `visibilityLabel="Revenue (USD)"` makes it announce "Revenue (USD)" — a deliberate WCAG 2.5.3 name-vs-label mismatch, accepted because the alternative (a runtime measurement deciding which source names the header) produced UNNAMED column headers ten separate times before it was deleted. A string header is always named by its own text; `visibilityLabel` does not override it. `column.id` is never used as a header name — a ReactNode header that names nothing and has no `visibilityLabel` leaves the column unnamed. That is a change from before #500, where such a header WAS named — from content, which swept in the resize handle, so it announced something like "Resize starred_col column". That name was the #500 bug, not a feature worth preserving. There is no dev warning for it: nothing static can tell a node that names itself (`<strong>Revenue</strong>`) from one that does not (an icon), so a warning fired on the commonest valid shape and was removed. Give any non-text header a `visibilityLabel`.
