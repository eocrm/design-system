...
- Focus advances as characters are typed. Boxes select their content on focus, so typing over a filled box replaces it, and a delivery shorter than the full code (a keystroke, or a short paste) spreads from whichever box is focused. A delivery of the full code — SMS/email autofill dumping it into whichever box happens to be focused — replaces the whole value instead of splicing in at that position. **Never cap a box at one character** — that truncates an autofilled code to its first digit.
