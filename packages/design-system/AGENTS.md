...
- **Shrinks to narrow containers without overflowing.** On viewports ≥ 640px the title column has a floor of `--page-header-title-min` (160px, `--measure-2xs`), capped at half the header's width — so the full 160px from ~320px containers up, and half the header below that; once title + actions don't fit, the actions give way first and their buttons wrap onto extra lines. The actions column is otherwise `auto`, so a wide header keeps all its actions on one line to the right (e.g. five ~120px actions in a 1000px header). `<PageHeader.Breadcrumb>` wraps too. **Residuals, measured in Chromium:** a single title word wider than the title column breaks mid-word (`overflow-wrap: anywhere`) — e.g. "Acme Corporation" with an `<Aside>` in 240–420px containers. With an `<Aside>` (32px avatar), a ~240px container still overflows the actions by ~14px (no overflow from ~280px; without an Aside none at 240px). Below a 640px viewport the header is a single column and neither applies.
