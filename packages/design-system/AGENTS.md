...
- `orientation`: `horizontal` (default), `vertical`, or `auto`. `auto` is vertical below 320px and horizontal at or above 320px based on the available Tabs/tab-strip width; use it with a collapsing `Split` so a fixed rail stays vertical until its pane stacks.
