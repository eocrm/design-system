...
- ⚠️ **_Migration:_ the lockup got shorter wherever `text-box-trim` is supported (Chrome/Safari; not Firefox).** Trimming the wordmark's half-leading is new — previous versions carried the font's full leading — so any lockup with a `subtext` or with `textPlacement="bottom"` renders measurably shorter at the same `size`. Measured with `text="eocrm" subtext="Free trial"` in Outfit (the playground's face — the DS ships no font, so `--logo-text-font` inherits and your own metrics will shift these figures): `sm` 30.8px → 24.0px, `md` 35.2px → 32.0px; `textPlacement="bottom"` at `lg` 74.4px → 59.6px, and at `sm` with a subtext 62.8px → 55.3px. Treat them as the shape of the change, not constants. The common side-by-side lockup **without** a subtext is unchanged (the mark sets the height). If you center a Logo in a fixed-height brand bar or auth splash, re-check that container — the lockup will re-center inside it. Firefox keeps the old heights, so the same component now differs by up to ~28% across browsers; size containers off the mark, not the lockup, if that matters to you.
