...
- **Bounds.** One rule governs both gestures: **the allowed range always contains where the event already is**, so a bound can refuse to move something further but never moves it the opposite way. Within that, a move keeps the start inside the visible `hourRange` — an unclamped upward drag would project into the previous calendar day — while the end runs past it freely, so an overnight booking keeps its duration; and a resize keeps the end within a day of the start, so an event can be extended past midnight but a runaway pointer delta can't roll through whole dates. An event that already starts before `hourRange` (its block renders clipped at the top) is _not_ pulled into the window.
