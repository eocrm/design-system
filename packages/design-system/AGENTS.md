...
- **`Rail.Header` brand area (default)** — a brand block with a header→nav divider out of the box: `--rail-header-padding` (default `0 0 var(--space-2)`) + `--rail-header-divider-width` (default `var(--border-width)`, color `--rail-border-color`). Matches the canonical rail brand with **no prop and no raw CSS**. For a bare header, override either token to `0`. The padding default is bottom-only on purpose: the rail's own `--rail-padding-x` / `--rail-padding-y` already inset the header, so adding a second inset pushes the brand off the nav items' icon column. When the rail is **collapsed** the header centers its content in the 56px track — render a mark-only brand there (read the state with `useRail()`); anything wider is clipped. _Migration:_ if you previously shimmed your own brand padding/divider on the header, remove it or it doubles.
