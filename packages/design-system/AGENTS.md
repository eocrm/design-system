...
- **Shrinks to narrow containers (down to ~300px) without overflowing.** On viewports ≥ 640px the title column has a floor of `--page-header-title-min` (160px, `--measure-2xs`); once title + actions don't fit, the actions give way first and their buttons wrap onto extra lines. The actions column is otherwise `auto`, so a wide header keeps all its actions on one line to the right (e.g. five ~120px actions in a 1000px header). `<PageHeader.Breadcrumb>` wraps too. **Residuals, measured in Chromium:** a single title word wider than the 160px floor breaks mid-word (`overflow-wrap: anywhere`) whenever the title sits at the floor — e.g. "Acme Corporation" with an `<Aside>` in 330/360/420px containers; in CONTAINERS narrower than ~300px with an `<Aside>` (~280px without) on a wide viewport, floor + gap + actions no longer fit and the actions overflow (measured: overflow at 280px with Aside and 240px without; none at 300px / 280px). Below a 640px viewport the header is a single column and neither applies.
...
- `politeness?: 'polite' | 'assertive'` — default `'polite'` (`role="status"`, waits its turn). `'assertive'` (`role="alert"`) interrupts immediately — reserve for errors. Both set `aria-atomic="true"`. Safe to switch together with the message (`politeness={error ? 'assertive' : 'polite'}`) — the role only flips while the region is empty, so old text is never announced as an alert; a politeness-only change re-announces the current message.
