...
describe('document background contract', () => {
  ...
  const tokens = readFileSync(resolve(__dirname, 'tokens.scss'), 'utf8');

  it('paints the body with the documented document token', () => {
    expect(reset).toMatch(/body\s*{[^}]*background:\s*var\(--document-background\);/s);
  });

  it('defaults the document token to the full-screen surface base', () => {
    expect(tokens).toContain('--document-background: var(--color-bg-subtle);');
  });
});
