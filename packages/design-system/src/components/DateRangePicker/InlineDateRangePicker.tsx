...
> {
   ...
   *
   * At `'minute'` the start/end time inputs are shown and editable even
   * before a range is picked (defaulting to `00:00` / `23:59`). Times
   * entered in the empty state are applied when the range is committed
      ...
      (date: Date) => {
        if (selectionStart == null) {
          ...
          if (granularity === 'minute') {
            ...
            // user may have set in the empty state. Round through `timeStep`
            // to keep the step-aligned invariant. Then clamp same-day end ≥
            // start.
