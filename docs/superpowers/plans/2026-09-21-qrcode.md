...
describe('encodeQr', () => {
  ...
  it('keeps the punch under a quarter of the symbol', () => {
    const { side, punch } = encodeQr(URL_VALUE, 'H')!;
...
export function encodeQr(value: string, level: QrCodeLevel): QrMatrix | null {
  ...
  try {
    ...
    // so an odd punch keeps `(side - punch) / 2` integral.
    return { side: count + QUIET * 2, path, punch: Math.ceil(count * PUNCH_RATIO) | 1 };
  } catch {
    return null;
  }
}
