...
// Token colors.
.delimiter {
  color: var(--liquid-editor-token-tag);
  opacity: 0.7;
}
.variable {
  color: var(--liquid-editor-token-variable);
}
.filter {
  color: var(--liquid-editor-token-filter);
}
.string {
  color: var(--liquid-editor-token-string);
}
.number {
  color: var(--liquid-editor-token-number);
}
.keyword {
  color: var(--liquid-editor-token-keyword);
...
> - `AutocompleteMenu.onSelect` ignores its args here and re-reads the active item via `acceptActive`. If you prefer mouse-click to select the _hovered_ item directly, set `activeIndex` to the clicked index first, then accept. Wire whichever is simpler; the unit test only asserts keyboard accept.
