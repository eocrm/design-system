...
| `packages/playground/src/pages/mockups/registry.ts`                   | `ComponentName` union member for `DemoLayout`                 |
...
- [ ] **Step 1b: Widen the `ComponentName` union**

`DemoLayout`'s `componentName` prop is typed `ComponentName` — a hand-maintained union in `packages/playground/src/pages/mockups/registry.ts`. Without this the demo page does not typecheck.

Add this line to that union, immediately **before** `| 'PasswordInput'`:

```ts
  | 'OtpInput'
```

Nothing else in that file needs to change: there is no exhaustive `Record<ComponentName, …>`, and a name absent from every mockup's `usesComponents` simply shows no cross-links.
