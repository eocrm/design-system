...
| Portal | `createPortal(surface, document.body)` + a `data-flowcanvas-maximized` marker | No `<Portal>` component exists; call `createPortal` directly, matching every other overlay. |
...
- When `maximized` is true, the **canvas surface** (the `role="application"` subtree containing the stage, SVG, nodes, and all three control zones) is rendered through `createPortal(..., document.body)` into a `position: fixed` container that fills `100dvw × 100dvh`, carrying `data-flowcanvas-maximized=""` and the `--z-flowcanvas-maximized` z-index.
- The **in-place root keeps its original box dimensions** (whatever height the parent gave it) while maximized, so the surrounding page does not reflow when the canvas pops out. On exit, the surface returns to the in-place root.
- **Viewport (pan/zoom) and selection are preserved across toggling.** `useViewport` state lives on the still-mounted `FlowCanvas` instance, so its values persist regardless of where the DOM is portaled; the existing `ResizeObserver` re-measures the new surface size and the transform re-applies. Entering maximize is a discrete button/key action, never mid-drag, so no in-progress gesture needs special handling — but if a connect drag is somehow active, it is cancelled on toggle.
...
The one non-trivial technical point is **preserving viewport/selection across the portal boundary** when entering/exiting maximize. Mitigation: keep the `FlowCanvas` component mounted (only the DOM subtree is portaled), so `useViewport`/selection React state survives; rely on the existing `ResizeObserver` to re-measure. The plan should include an explicit test that pans/zooms, toggles twice, and asserts the transform is unchanged, to catch any accidental remount/reset.
