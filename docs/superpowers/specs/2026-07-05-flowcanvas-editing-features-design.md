...
2. **`allowConnections={false}`** — a narrow gate disabling connection _creation_ and _rewiring_ while keeping node drag/move/delete/select.
...
- `onEndpointPointerDown`: sets `connect = { from: fixed, mode:'pointer', pointerId, target:null, cursor:null, edgeId, end, fixed }` where `fixed` is the _other_ endpoint (dragging `target` keeps `source` fixed, and vice-versa). Root captures the pointer (same as connect).
...
- Dragging an edge endpoint to create a _branch_ (duplicating) — rewire moves, never copies.
